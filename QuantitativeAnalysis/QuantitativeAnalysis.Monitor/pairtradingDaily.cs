...
namespace QuantitativeAnalysis.Monitor
{
    ...
    public class pairtradingDaily
    {
        ...
        private bool bollingerBrand1(List<StockTransaction> underlying1, List<StockTransaction> underlying2, double[] signal, int duration, List<BollingerBandwithPrice> myboll, ref List<OneByOneTransaction> data,ref List<netvalueDaily>netvalueList)
        {
            ...
            for (int i = 1; i < length; i++)
            {
                ...
                else if (position1 > 0 && position2 < 0 &&  trade == true)
                {
                    ...
                    //}
                    //追踪止损
                    if (trailingStop==true || holdDays > 2*duration)
                    {
                        cash += position1 * underlying1AvgPrice * (1 - slipRatio) + position2 * underlying2AvgPrice * (1 + slipRatio);
                        position1 = 0;
                        position2 = 0;
